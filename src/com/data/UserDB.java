...
public class UserDB {
    ...
    public static ArrayList<Dashboard> getDashboardList(int userID) {
        ...
        String query =
        "select ua.doctorID, ua.userID, status, concat(d.firstName,' ',d.lastName)"
        + " as doctorName,availDate,availTime,hospitalName,hospitalAddress"
        + "  as hospitalAddress,d.specialization from userappointment ua inner join availability a "
        + " on ua.availID = a.availID inner join doctor d on d.doctorID = ua.doctorID where ua.userId ="+userID;
        System.out.println((query));
        try {
        	java.sql.Connection con = Connection.getConnection(query);
            ps = con.prepareStatement(query);
